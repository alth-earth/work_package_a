...
def build_parser() -> argparse.ArgumentParser:
    ...
    ingest.add_argument("--quality", choices=[item.value for item in QualityFlag], default="good")
    ...
    replay.add_argument("--horizon-hours", type=int, default=24)
    replay.add_argument("--max-memory-mb", type=float, default=512)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "ingest":
        ...
        evidence = IssueTimeEvidence(
            ...
            authoritative=args.quality == QualityFlag.GOOD.value,
        )
    ...
    if args.command == "replay":
        ...
        cache = PartitionedABCache(max_memory_mb=args.max_memory_mb)
        service = WorkPackageA(
            source=LocalArchiveSource(args.data_root), clock=clock, cache=cache
        )
    ...
    if args.command == "doctor":
        report = inspect_archive(args.data_root)
        print(json.dumps({
            "ok": report.ok,
