...
语义。本轮 A–B–C 主线 bundle 固定为恰好 12 类；两类可选层独立采集和报告，失败
不得阻塞基线，也不得静默拼入主线。
...
工作包 A 额外支持 `vessel_traffic` 可选动态层。该层来自航道通行情况模拟模型，主变量为
`vessel_traffic_risk`，取值范围为 0-1。其作用是为工作包 B 的综合风险预测补充通航拥挤度、
邻近船舶干扰和航道活跃程度等实时通航状态特征。

B 侧使用该层时应遵循以下规则：

- 将 `vessel_traffic` 作为可选风险因子读取；
- 不将该层缺失视为 A 的 12 类必需环境数据失败；
- 保留 A manifest 中的 `quality_flag`、`issue_time`、`valid_time` 和来源元数据；
- 默认把 `quality_flag=suspect` 理解为“模拟增强数据”，不能当作权威 AIS 实测数据；
- 若该层参与综合风险计算，应在 B 的 `source_summary` 或等价溯源字段中记录。

补充该层是为了处理北极航线历史通航数据难以稳定开放获取的问题。它不改变 A/B/C 原有正式接口，
而是在不破坏主线的情况下，为综合风险模型提供更接近实时通航情况的动态输入。
